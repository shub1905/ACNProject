...
bool receivePacket(char *data) {
  int n = recvfrom(sock,data,sizeof(tcp_header),0,(struct sockaddr *) &remoteAddress,&remoteLen);
  ...
  int n = recvfrom(sock, data+sizeof(tcp_header), MAX_MSG-sizeof(tcp_header), 0, (struct sockaddr *) &remoteAddress, &remoteLen);
  if (n<0) {
    return false;
  }
  return true;
...
class bitmap
{
  ...
  bool bitmapSet(int pos, bool p, int size = BUF_SIZE_OS) {
    if (pos >= size) {
      return false;
    }
    int a = pos/8;
  ...
  void bitmapreset(int size = BUF_SIZE_OS) {
